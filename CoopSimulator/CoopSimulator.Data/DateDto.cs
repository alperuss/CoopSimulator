    ...
    public class DateDto
    {
        ...
        public DateTime Date
        {
            ...
            set
            {
                ...
                {
                    if (_datePeriod == Enums.DatePeriod.Day && 
                        (value - _lastTriggeredDate).TotalDays>=_datePeriodValue)
                    {
                        _lastTriggeredDate = value;
                        _datePeriodTriggered?.Invoke(null,null);
                    }
                    else if (_datePeriod == Enums.DatePeriod.Week && 
                             Helper.DateHelper.WeekDifferenceBetweenTwoDate(_lastTriggeredDate,value) >=_datePeriodValue)
                    {
                        _lastTriggeredDate = value;
                        _datePeriodTriggered?.Invoke(null, null);
                    }
                    else if (_datePeriod == Enums.DatePeriod.Month && 
                             Helper.DateHelper.MonthDifferenceBetweenTwoDate(_lastTriggeredDate, value) >= _datePeriodValue)
                    {
                        _lastTriggeredDate = value;
                        _datePeriodTriggered?.Invoke(null, null);
                    }
                    else if (_datePeriod == Enums.DatePeriod.Year && 
                             Helper.DateHelper.YearDifferenceBetweenTwoDate(_lastTriggeredDate, value) >= _datePeriodValue)
                    {
                        _lastTriggeredDate = value;
                        _datePeriodTriggered?.Invoke(null, null);
                    }
